"""
HTTP API Server for the XiaoAI speaker
...
class APIServer:
    """HTTP API Server to control the XiaoAI speaker remotely"""
    ...
    async def handle_tts_doubao(self, request: web.Request) -> web.Response:
        ...
        Request body:
            {
                "text": "你好",                    # required
                "api_key": "your_api_key",         # optional (uses config if not provided)
        ...
        try:
            ...
            # Get API Key from request or config (new-console auth)
            tts_config = self.config.get_app_config("tts.doubao", {})

            api_key = str(data.get("api_key") or tts_config.get("api_key") or "")
            ...
            if not api_key:
                return web.json_response(
                    {"success": False, "error": "Doubao API Key not configured. Provide api_key in request or config.py"},
                    status=400
                )
