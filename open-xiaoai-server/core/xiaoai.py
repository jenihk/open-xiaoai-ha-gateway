...
class XiaoAI:
    ...
    def on_input_data(cls, data: bytes):
        # Feed raw mic audio to all readers (VAD/ASR and KWS).
        # Input gain is applied inside the KWS pipeline only, so the
        # conversation VAD + ASR always see clean, unclipped audio.
        GlobalStream.input(data)
